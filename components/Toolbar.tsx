...
export default function Toolbar(props: Props) {
  ...
  return (
    <div className="toolbar">
      <strong>甍AI Geometry Editor</strong>
      <span style={{ opacity: 0.6, fontSize: 12 }}>e0.3</span>

      <label className="file" onClick={() => fileRef.current?.click()}>
        図面を読み込む
        <input
          ref={fileRef}
          type="file"
          ...
          onChange={(e) => {
            ...
            if (f) props.onLoadBackground(f);
            e.target.value = '';
          }}
        />
      </label>

      <button onClick={props.onNewPolygon}>＋ 新規 Polygon</button>
